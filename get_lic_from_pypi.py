...
def get_lic(package_with_version): # pass the package name in format bs4==0.0.1
    ...
    if lic.status_code != requests.codes.ok: # status code 200 != requests code.
...
pd_packages = pd.DataFrame(pkg_lic_info, columns=['Package Name','License Type','Description'])

print pd_packages
#
pd_packages.to_csv('pd_packages.csv')

remove_unknown_lic = pd_packages["License Type"].str.contains("UNKNOWN| ")
